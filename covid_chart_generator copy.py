...
def fetch_zoe_data() -> pd.DataFrame:
    ...
    for i in range(10):
        try:
            ...
            raise ImportError("gcsfs package not installed, required") from missing_gcsfs
...
def process_zoe(raw_zoe: pd.DataFrame) -> pd.DataFrame:
    if not raw_zoe.empty:
        ...
        zoe_dataframe.index=pd.to_datetime(zoe_dataframe.index, dayfirst=True)
        zoe_dataframe['Midlands'] = zoe_dataframe['West Midlands'] + zoe_dataframe['East Midlands']
        zoe_dataframe['North East and Yorkshire'] = (zoe_dataframe['North East'] +
...
def fetch_deaths_data():
    ...
    deaths_df = call_gov_api(callname='deaths', filters=['areaType=region'],
        structure=deaths_query)
    deaths_df.date = pd.to_datetime(deaths_df.date)
    return deaths_df

def process_deaths_data(deaths_df: pd.DataFrame) -> pd.DataFrame:
    ney = deaths_df[(deaths_df.region=='North East') | (
        deaths_df.region=='Yorkshire and The Humber')].groupby('date').sum()
    midlands = deaths_df[(deaths_df.region=='East Midlands') | (
        deaths_df.region=='West Midlands')].groupby('date').sum()
...
def fetch_healthcare_data(area_name: str) -> pd.DataFrame:
    ...
    return call_gov_api(callname=f'healthcare ({area_name})',
        filters=areas[area_name], structure=healthcare_query)
...
def process_cases_data(cases_dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    u60_df = cases_dataframe[cases_dataframe.age=='00_59'].pivot_table(
        index='date', columns='region').cases
    o60_df = cases_dataframe[cases_dataframe.age=='60+'].pivot_table(
        index='date', columns='region').cases
...
if __name__ == '__main__':
    ...
    regions_to_use.insert(0, regions_to_use.pop(regions_to_use.index('England')))
